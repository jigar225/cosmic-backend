...
const gradeSelectCols = `id, grade_method_id, title, display_order, numeric_equivalent,
...
func scanGrade(s gradeScanner) (domain.Grade, error) {
	var g domain.Grade
	err := s.Scan(
		&g.ID,
		&g.GradeMethodID,
...
func (r *GradeRepo) ListByGradeMethod(ctx context.Context, gradeMethodID int64, visibleOnly bool) ([]domain.Grade, error) {
	query := `SELECT ` + gradeSelectCols + ` FROM grades WHERE grade_method_id = $1`
	args := []interface{}{gradeMethodID}
	if visibleOnly {
		query += " AND is_visible = true"
	}
	query += " ORDER BY display_order, title, id"
...
func (r *GradeRepo) ListAll(ctx context.Context, gradeMethodID *int64) ([]domain.Grade, error) {
	query := `SELECT ` + gradeSelectCols + ` FROM grades WHERE 1=1`
	args := []interface{}{}
	n := 1
	...
	query += " ORDER BY grade_method_id, display_order, title, id"
...
func (r *GradeRepo) Create(ctx context.Context, in domain.CreateGradeInput) (domain.Grade, error) {
	...
	row := r.pool.QueryRow(ctx, `
		INSERT INTO grades (grade_method_id, title, display_order, numeric_equivalent,
		                    age_range_start, age_range_end, academic_stage, is_visible)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING `+gradeSelectCols,
		in.GradeMethodID,
		in.Title,
		in.DisplayOrder,
...
func (r *GradeRepo) GetByID(ctx context.Context, id int64) (domain.Grade, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT `+gradeSelectCols+` FROM grades WHERE id = $1`, id)
	g, err := scanGrade(row)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
// Returns ErrGradeHasDependents if any subjects, user_default, or generated_content reference this grade.
